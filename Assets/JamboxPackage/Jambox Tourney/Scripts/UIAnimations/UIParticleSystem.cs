...
public class UIParticleSystem : MonoBehaviour {
    ...
    public float InspectorLifeTime;
    /// <summary>
    ...
    public int MaxResolutinHeight;

    /*
    // Use this for initialization
    void Start () {
        if (FindObjectOfType<MatchmakingPanel>() != null)
        {
            Speed -= (MaxResolutinHeight - Screen.height) * 0.325f;
            Gravity += (MaxResolutinHeight - Screen.height) * 2.130f;
        }else if (FindObjectOfType<DuelResultPanel>() != null)
        {
            Speed -= (MaxResolutionWidth - Screen.width) * 0.025f;
            Gravity += (MaxResolutinHeight - Screen.height) * 2.130f;
            Lifetime -= (MaxResolutinHeight - Screen.height) * 0.0001f;
        }
    }*/
    ...
    private IEnumerator CoParticleFly(Image particle)
    {
        ...
        while (particleLifetime < Lifetime)
        {
            ...
            if (AddForceTowards)
            {
                if (particleLifetime >= timeToApply)
                {
                    particle.transform.localPosition += (emissonAngle * SpeedOverLifetime.Evaluate(particleLifetime / Lifetime) * Speed + gravityForce + Force * forceDirection) * Time.deltaTime;
                }
                else
                {
                    particle.transform.localPosition += (emissonAngle * SpeedOverLifetime.Evaluate(particleLifetime / Lifetime) * Speed + gravityForce) * Time.deltaTime;
                }
            }
            else
            {
